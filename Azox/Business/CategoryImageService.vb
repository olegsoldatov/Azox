...
Public Class CategoryImageService
    ...
    Private ReadOnly Property LargeConfiguration As ImageConfiguration
    Private ReadOnly Property MediumConfiguration As ImageConfiguration
    ...
    Public Sub New(imageManager As ImageManager)
        ...
        LargeConfiguration = New ImageConfiguration With {.Width = 1200, .Height = 255, .Mode = StretchMode.UniformToFill, .Background = Color.White}
        MediumConfiguration = New ImageConfiguration With {.Width = 544, .Height = 306, .Mode = StretchMode.UniformToFill, .Background = Color.White}
        SmallConfiguration = New ImageConfiguration With {.Width = 320, .Height = 180, .Mode = StretchMode.UniformToFill, .Background = Color.White}
    End Sub
    ...
    Public Async Function UploadAsync(entity As IPictorial, imageFile As (InputStream As Stream, ContentType As String)) As Task Implements IImageService.UploadAsync
        ...
        If Not (IsNothing(imageFile.InputStream) And IsNothing(imageFile.ContentType)) Then
            ...
            With image
                ...
                .Large = If(IsNothing(LargeConfiguration), Nothing, ImageUtility.Generate(imageFile.InputStream, imageFile.ContentType, LargeConfiguration.Width, LargeConfiguration.Height, LargeConfiguration.Mode, LargeConfiguration.Background))
                .Medium = If(IsNothing(MediumConfiguration), Nothing, ImageUtility.Generate(imageFile.InputStream, imageFile.ContentType, MediumConfiguration.Width, MediumConfiguration.Height, MediumConfiguration.Mode, MediumConfiguration.Background))
